...
public class MainActivityTest {
    ...
    @Test
    public void testEnterShowActivity() {
        // Add city
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Lambda"));
        onView(withId(R.id.button_confirm)).perform(click());

        // click on Lambda
        onData(is(instanceOf(String.class))).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());

        // assert we are in ShowActivity
        onView(withId(R.id.text_city_name)).check(matches(isDisplayed()));
    }

    @Test
    public void testCityNameShowActivity() {
        // Add city
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Blambda"));
        onView(withId(R.id.button_confirm)).perform(click());

        // click on Blambda
        onData(is(instanceOf(String.class))).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());

        // assert Blambda is shown in ShowActivity
        onView(withId(R.id.text_city_name)).check(matches(withText("Blambda")));
    }

    @Test
    public void testBackButtonShowActivity() {
        // Add a city
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Bobamada"));
        onView(withId(R.id.button_confirm)).perform(click());
        ...
        // assert we are back to MainActivity
